#include "P18h_vTrackTowerMaker.h"
...
ClassImp(P18h_vTrackTowerMaker)

P18h_vTrackTowerMaker::P18h_vTrackTowerMaker (
    ...
    print_level   {0}
{
    cout << " ------------------- star_dB TIGER " << starDb << endl;


};

vector<myTrack> P18h_vTrackTowerMaker::makeVecMyTracks() {
    vector<myTrack>  vTracks;
    ...
    if (print_level == 1) cout << " mBField " << mBField_tesla << endl;
    if (print_level == 1) cout << " bemc_radius " << bemc_radius << endl;
    ...
    for (int i_track{0}; i_track< (int) mMuDst->numberOfPrimaryTracks(); ++i_track){
        ...
        if (print_level == 1) cout << " pt " << pt << endl;
        if (pt < 0.2) continue;
        if (print_level == 1) cout << " debug C1 " << endl;

        bool isTofMatch = (muTrack->index2BTofHit() >= 0);
        bool isBEMCMatch = (muTrack->matchBEMC()); // update later
        if (print_level == 1) cout << " isTofMatch " << isTofMatch << endl;
        ...
        if ( mEmcPosition->projTrack(&bemc_pos, &bemc_mom, gTrk, mBField_tesla, bemc_radius) ) {
            if (print_level == 1) cout << " alpha 0   bemc_pos " << bemc_pos << endl;
            if (print_level == 1) cout << "  bemc_mom " << bemc_mom << endl;
...
vector<myTower> P18h_vTrackTowerMaker::makeVecMyTowers() {
    vector<myTower> vTowers;
    ...
    if (mEmcCollection == nullptr) {
        cout << "Fatal error in P18h_vTrackTowerMaker: can't get EmcCollection." << endl;
        return vTowers;
    }
    ...
    for (UInt_t i = 1; i <= mBEMCModules; ++i) {
        ...
        for (UInt_t j = 0; j < mEmcTowerHits.size(); ++j) {
            ...
            while (towerPhi < Pi) towerPhi += TwoPi;

            double towerEt { towerEnergy / TMath::CosH(towerEta) };

            vTowers.push_back({towerID, 
